...
export default function RoadmapNodeClient({
  ...
  const fetchData = async () => {
    try {
      ...
      setNode(data);
      await fetchProgressForNode(data.id);
    } catch (e) {
      console.error(e);
    } finally {
      setLoading(false);
  ...
  const fetchProgress = async () => {
    await fetchProgressForNode(nodeId);
  };

  const fetchProgressForNode = async (id: string) => {
    try {
      const progRes = await clientFetch('/api/v1/roadmaps/me');
      if (progRes.ok) {
        const progData = await progRes.json();
        setProgress(progData);
        setIsCompleted(progData.some((p: any) => p.nodeId === id));
      }
    } catch (e) {
      console.error(e);
    }
