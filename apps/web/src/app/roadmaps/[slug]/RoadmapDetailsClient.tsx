...
export default function RoadmapDetailsClient({
  ...
  slug: string;
  initialRoadmap: any | null;
}) {
  const router = useRouter();
  const [roadmap, setRoadmap] = useState<any>(initialRoadmap);
  const [progress, setProgress] = useState<any[]>([]);
  const [loading, setLoading] = useState(initialRoadmap === null);
  ...
  const fetchData = async () => {
    try {
      ...
      await fetchProgressForRoadmap(rmData.id);
    } catch (e) {
      console.error(e);
    } finally {
      setLoading(false);
  ...
  const fetchProgressForRoadmap = async (roadmapId?: string) => {
    try {
      ...
      if (progRes.ok) {
        ...
        setProgress(progData.filter((p: any) => p.roadmapId === roadmapId));
      }
    } catch (e) {
      console.error(e);
    }
